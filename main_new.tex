...
\begin{sidewaysfigure}
    ...
    \begin{tikzpicture}[
        font=\footnotesize,
        box/.style={draw, rounded corners, align=center, inner sep=5pt, text width=3.4cm, fill=black!3},
        ...
        % Left-to-right, three lanes (categorical / ordinal / continuous)
        \node[decision] (dtype) {Data type?};

        \node[box, right=20mm of dtype, yshift=18mm] (cat) {Categorical\\(nominal/binary)};
        \node[box, right=20mm of dtype] (ord) {Ordinal\\(e.g., Likert)};
        \node[box, right=20mm of dtype, yshift=-18mm] (cont) {Continuous\\(interval/ratio)};
        ...
        \node[decision, right=20mm of cat] (rq_cat) {Research\\question?};
        \node[decision, right=20mm of ord] (rq_ord) {Research\\question?};
        \node[decision, right=20mm of cont] (rq_cont) {Research\\question?};

        \draw[edge] (cat) -- (rq_cat);
        \draw[edge] (ord) -- (rq_ord);
        \draw[edge] (cont) -- (rq_cont);

        % Differences vs association (categorical)
        \node[box, right=20mm of rq_cat, yshift=10mm] (cat_diff) {Differences\\between groups};
        \node[box, right=20mm of rq_cat, yshift=-10mm] (cat_assoc) {Association};
        \draw[edge] (rq_cat) -- node[lab, above]{differences} (cat_diff);
        \draw[edge] (rq_cat) -- node[lab, below]{association} (cat_assoc);
        \node[box, right=20mm of cat_diff] (cat_diff_tests) {$\chi^2$ / Fisher\\(independent)\\McNemar / Cochran's Q\\(paired)};
        \node[box, right=20mm of cat_assoc] (cat_assoc_tests) {$\chi^2$ / Fisher\\(or logistic models)};
        \draw[edge] (cat_diff) -- (cat_diff_tests);
        \draw[edge] (cat_assoc) -- (cat_assoc_tests);

        % Differences vs association (ordinal)
        \node[box, right=20mm of rq_ord, yshift=10mm] (ord_diff) {Differences\\between groups};
        \node[box, right=20mm of rq_ord, yshift=-10mm] (ord_assoc) {Association};
        \draw[edge] (rq_ord) -- node[lab, above]{differences} (ord_diff);
        \draw[edge] (rq_ord) -- node[lab, below]{association} (ord_assoc);
        \node[box, right=20mm of ord_diff] (ord_diff_tests) {2 groups:\\Mann--Whitney (indep.)\\Wilcoxon (paired)\\$>2$ groups:\\Kruskal--Wallis\\Friedman (paired)};
        \node[box, right=20mm of ord_assoc] (ord_assoc_tests) {Spearman};
        \draw[edge] (ord_diff) -- (ord_diff_tests);
        \draw[edge] (ord_assoc) -- (ord_assoc_tests);

        % Differences vs association (continuous)
        \node[box, right=20mm of rq_cont, yshift=10mm] (cont_diff) {Differences\\between groups};
        \node[box, right=20mm of rq_cont, yshift=-10mm] (cont_assoc) {Association};
        \draw[edge] (rq_cont) -- node[lab, above]{differences} (cont_diff);
        \draw[edge] (rq_cont) -- node[lab, below]{association} (cont_assoc);
        \node[box, right=20mm of cont_diff] (cont_diff_tests) {2 groups:\\t-test (indep./paired)\\(or rank-based alternative)\\$>2$ groups:\\ANOVA\\(or non-parametric alternative)};
        \node[box, right=20mm of cont_assoc] (cont_assoc_tests) {Pearson\\(if assumptions)\\or Spearman};
        \draw[edge] (cont_diff) -- (cont_diff_tests);
        \draw[edge] (cont_assoc) -- (cont_assoc_tests);
    \end{tikzpicture}
    }%
    \caption{Alternative decision tree (TikZ draft) for selecting a statistical test (left-to-right layout).}
