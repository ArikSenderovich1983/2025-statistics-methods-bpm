...
\abstract{Business Process Management (BPM) research increasingly relies on empirical evidence: comparing algorithms and tools, evaluating interventions, and studying perceptions and behavior of practitioners. In such settings, statistics provide a principled way to reason from limited observations, quantify uncertainty, and avoid over-interpreting random variation.

This chapter introduces statistical inference for BPM researchers with an emphasis on hypothesis testing. We present a practical workflow for formulating hypotheses, selecting suitable tests based on data type and study design, and reporting results transparently. Two running examples (paired usefulness ratings of modeling notations and the relationship between algorithm runtime and a log characteristic) illustrate how to interpret $p$-values, complement them with effect sizes, and proceed when results are not statistically significant. Because Likert-type scales are common in BPM studies, we discuss an ordinal-first treatment and corresponding non-parametric tests. We also highlight typical pitfalls, including misuse of tests, over-reliance on significance thresholds, and multiple comparisons, and provide guidance for mitigation.}
