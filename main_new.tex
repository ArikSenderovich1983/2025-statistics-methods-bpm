...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={draw, rounded corners, align=center, text width=4.0cm, inner sep=4pt, fill=black!3},
        edge from parent/.style={draw, -{Stealth[length=2mm]}, thick},
        level 1/.style={sibling distance=7.8cm, level distance=2.0cm},
        level 2/.style={sibling distance=4.5cm, level distance=2.0cm}
    ]
        \node[decision]{Outcome /\\ data type?}
            child{ node[box]{Categorical\\(nominal/binary)}
                child{ node[decision]{Research\\ question?}
                    child{ node[box]{Differences between groups\\$\chi^2$ / Fisher}
                        edge from parent node[above]{differences}
                    }
                    child{ node[box]{Association\\$\chi^2$ / Fisher\\(or logistic models)}
                        edge from parent node[above]{association}
                    }
                }
                edge from parent node[above]{categorical}
            }
            child{ node[box]{Ordinal\\(e.g., Likert)}
                ...
                edge from parent node[above]{ordinal}
            }
            child{ node[box]{Continuous\\(interval/ratio)}
                ...
                edge from parent node[above]{continuous}
            };
    \end{tikzpicture}
